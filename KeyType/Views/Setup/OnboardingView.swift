...
struct OnboardingView: View {
    ...
    private var footer: some View {
        ...
        case .done:
            HStack {
                Spacer()
                Button("Start Using Glide") { finish() }
                    .buttonStyle(.borderedProminent)
                    .controlSize(.large)
            }
        case .permissions:
    ...
    private var permissionsStep: some View {
        StepHeader(
            title: "Enable Glide",
            subtitle: "Glide needs a couple of permissions to read the focused field and accept completions."
        )
        VStack(spacing: 10) {
            PermissionCard(
                kind: .accessibility,
            ...
            PermissionCard(
                ...
                requirement: .required,
                explanation: "Detects configured global accept keys so Glide can insert a completion.",
            ...
            PermissionCard(
                ...
                explanation: "Optional. Lets Glide read on-screen text (OCR) from the focused window as extra context, when you enable it in Privacy. Glide works without it.",
    ...
    private var predictionsStep: some View {
        StepHeader(
            ...
            subtitle: "macOS has its own inline prediction that draws ghost text too. Turn it off so it doesn't collide with Glide."
        )
    ...
    private var doneStep: some View {
        VStack(spacing: 18) {
            ...
            Label("Find Glide in your menu bar.", systemImage: "menubar.arrow.up.rectangle")
                .font(.footnote)
