...
/// Owns the non-activating overlay that visually anchors KeyType's drag helper inside System
/// Settings. Callers ask for "present / update / hide" behavior without learning anything about
/// `NSPanel` configuration or animation timing.
final class PermissionOverlayWindowController: NSWindowController {
    ...
    /// Approximates a critically damped spring so the overlay feels deliberate rather than floaty.
...
private final class PermissionOverlayContentView: NSView {
    ...
    private func setup(hostApp: PermissionHostApp, permission: PermissionKind) {
    ...
    private func title(hostApp: PermissionHostApp, permission: PermissionKind) -> NSAttributedString {
        NSAttributedString(
